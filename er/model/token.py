...
@argmethod.wrap
async def get_most_recent_session_by_uid(uid: objectid.ObjectId):
  """Get the most recent session by uid."""
  coll = db.coll('token')
  doc = await coll.find_one({'uid': uid,
                             'token_type': TYPE_SESSION},
                            sort=[('update_at', -1)])
  return doc


@argmethod.wrap
async def get_session_list_by_uid(uid: objectid.ObjectId):
...
@argmethod.wrap
async def delete_by_uid(uid: objectid.ObjectId):
